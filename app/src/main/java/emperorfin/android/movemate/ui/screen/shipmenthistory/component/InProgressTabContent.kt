...
fun InProgressTabContent() {
    val shipmentItems = FakeDataSource.getInProgressShipmentItems()

    LazyColumn(
        modifier = Modifier
            .fillMaxSize()
            .padding(horizontal = dimensionResource(id = R.dimen.in_progress_tab_content_lazy_column_padding_horizontal_16))
    ) {
        item {
            TextTile(
                title = R.string.label_shipments,
                titleColor = BlackFf70737a,
                titleFontSize = dimensionResource(id = R.dimen.in_progress_tab_content_text_tile_title_font_size_23).value.sp,
                titleFontWeight = FontWeight.SemiBold,
            )

            Spacer(modifier = Modifier.height(height = dimensionResource(id = R.dimen.in_progress_tab_content_space_height_8)))
        }
